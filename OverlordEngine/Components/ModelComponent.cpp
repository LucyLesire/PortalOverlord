...
void ModelComponent::Initialize(const SceneContext& sceneContext)
{
	...
	PortalRenderer::Get()->UpdateMeshFilter(sceneContext, m_pMeshFilter);
}
...
void ModelComponent::PortalDraw(const SceneContext& sceneContext)
{
	PortalRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, GetTransform()->GetWorld(), m_pAnimator->GetBoneTransforms());
}
