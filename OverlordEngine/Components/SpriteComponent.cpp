...
void SpriteComponent::Draw(const SceneContext& sceneContext)
{
	...
	const auto pos = GetGameObject()->GetTransform();

	SpriteRenderer::Get()->AppendSprite(m_pTexture, XMFLOAT2{ pos->GetPosition().x, pos->GetPosition().y }, 
		m_Color, m_Pivot, XMFLOAT2{ pos->GetScale().x, pos->GetScale().y }, MathHelper::QuaternionToEuler(pos->GetRotation()).z, pos->GetPosition().z);
