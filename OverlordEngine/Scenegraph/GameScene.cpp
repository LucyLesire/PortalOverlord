...
void GameScene::DrawPortals(RenderTarget* rt)
{
...
#pragma region POST-PROCESSING PASS
	...
	if (m_PostProcessingMaterials.size() > 0)
	{
		...
		auto prevRenderTarget = m_pGame->GetRenderTarget();
		auto initRenderTarget = prevRenderTarget;
		...
		if(prevRenderTarget != initRenderTarget)
		{
			...
			SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, prevRenderTarget->GetColorShaderResourceView(), XMFLOAT2{0,0});
		}
...
void GameScene::AddPostPortalEffect(PostPortalMaterial* pMat)
{
	m_PostPortalMaterials.push_back(pMat);
}
