...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	auto proj = XMMatrixOrthographicLH(100.f * sceneContext.aspectRatio, 100.f, 0.1f, 500.f);
	...
	auto eyePos = XMLoadFloat4(&sceneContext.pLights->GetDirectionalLight().position);
	auto focusPos = eyePos + XMLoadFloat4(&sceneContext.pLights->GetDirectionalLight().direction);
	auto worldUp = XMFLOAT4(0.f, 1.f, 0.f, 0.f);
	auto upPos = XMLoadFloat4(&worldUp);
	auto view = XMMatrixLookAtLH(eyePos, focusPos, XMLoadFloat4(&sceneContext.pLights->GetDirectionalLight().direction));
	...
	XMStoreFloat4x4(&m_LightVP, XMMatrixMultiply(view, proj));
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	{
		meshType = ShadowGeneratorType::Skinned;
		m_pShadowMapGenerator->SetVariable_MatrixArray(L"gBones", &meshBones[0]._11, int(meshBones.size()));
	}
	else
	{
		meshType = ShadowGeneratorType::Static;
