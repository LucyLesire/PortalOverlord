...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	//topLeft = float3(origin.x - (size), origin.y + (size), origin.z);
	//topRight = float3(origin.x + (size), origin.y + (size), origin.z);
	//bottomLeft = float3(origin.x - (size), origin.y - (size), origin.z);
	//bottomRight = float3(origin.x + (size), origin.y - (size), origin.z);
