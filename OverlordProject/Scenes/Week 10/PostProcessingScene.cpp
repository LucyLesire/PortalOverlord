...
void PostProcessingScene::Initialize()
{
	...
	m_pPostPortal = MaterialManager::Get()->CreateMaterial<PostPortalMaterial>();

	AddPostProcessingEffect(m_pPostGrayscale);
	AddPostProcessingEffect(m_pPostBlur);
	//AddPostProcessingEffect(m_pPostPortal);
	AddPostPortalEffect(m_pPostPortal);
	...
	//pStaticCameraComponent->GetTransform()->Translate(500.f, 25.f, 0.f);
	//pStaticCameraComponent->GetTransform()->Rotate(90.f, 0.f, 0.f, true);
	//m_pPortalCam = pStaticCameraComponent;
...
void PostProcessingScene::OnGUI()
{
	...
	isEnabled = m_pPostPortal->IsEnabled();
	ImGui::Checkbox("PORTAL", &isEnabled);
	m_pPostPortal->SetIsEnabled(isEnabled);
}
